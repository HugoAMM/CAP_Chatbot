...
export class FlightBookingRecognizer {
    ...
    getDocumentoEntities(result) {
        let documentoValue;
        console.log(result.entities.$instance)
        if (result.entities.$instance.documento) {
            documentoValue = result.entities.$instance.documento[0].text;
        }
        return { documento: documentoValue};
    } 
}
